...
function DevicesSlider(props) {
  ...
  return (
     ...
     {TABS_KEYS.map(key =>
         <div key={key} role="tabpanel" sliderRef={sliderRef}
         className={'section__panel' + (key === activeTab ? '' : ' section__panel_hidden')} aria-hidden={key === activeTab ? 'false' : 'true'} id={`panel_${key}`} aria-labelledby={`tab_${key}`}>
